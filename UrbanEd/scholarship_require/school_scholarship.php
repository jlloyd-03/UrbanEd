<!DOCTYPE html>
    <head>
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">
        <link rel="stylesheet" href="assets/scholarship/scholarship.css">
        <link rel="stylesheet" href="assets/scholarship/schools.css">
    </head>
    <body>
        <main class="page" tabindex="-1">
            <div class="page-feature">
                <div class="page-divider"></div>
            </div>

            <div class="page-header">
                <nav class="nav-header">
                    <a href="index.php">HOME</a>
                    <i class="fas fa-angle-right arrow"></i>
    
                    <a href="scholarship_homepage.php">SCHOLARSHIP</a>
                    <i class="fas fa-angle-right arrow"></i>
                    
                    <a href="scholarships.php"><span class="blue-text">SCHOOL SCHOLARSHIP</span></a>
                </nav>
            </div>

            <div class="page-row">
                <div class="page-container">
                    <div class="page-content">
                        <div class="typography">
                            <h1>School Scholarship</h1>
                        </div>
                        <div class="content">
                            <div class="intro">
                                <p>At UrbanEd, we see scholarships offered by schools as more than just financial aid, they’re gateways to growth, opportunity, and discovery.</p>
                            </div>
                            <div class="description">
                                <div class="d1">
                                    <p>These scholarships allow students to thrive in academic environments where they can explore their interests, push their limits, and uncover new strengths. Whether awarded for academic excellence, leadership, or unique talents, school scholarships open the door to a richer, more fulfilling college experience.</p>
                                </div>
                                <div class="d2">
                                    <p>UrbanEd helps connect students to these opportunities—so they can focus less on the cost and more on developing the skills, values, and knowledge that will shape their future.</p>
                                </div>
                            </div>
                        </div>
<!-- schools in bacolod -->
                    <article class="article-schools">
                        <figure class="pic-container">
                            <img class="city-pic" 
                                src="img/city_logos/bacolod.png" 
                                alt="Scholarship Opportunities">
                        </figure>
                        <div class="art-des">
                            <div class="art-title">
                                <h2>Bacolod City</h2>
                            </div>
                            <div class="des-art">
                                <p>Browse available scholarships from local schools inside Bacolod, including eligibility requirements, application steps, and key details to help you start your journey..</p>

                                <div class="schools hidden">
                                    <div class="school">
                                        <img src="img/school_logos/usls.png" alt="USLS Logo" class="school-logo">
                                        <div class="school-details">
                                            <h3>University of St. La Salle</h3>
                                        </div>
                                    </div>

                                    <div class="school">
                                        <img src="img/school_logos/stiwnu.jpg" alt="STI Logo" class="school-logo">
                                        <div class="school-details">
                                            <h3>STI West Negros University</h3>
                                        </div>
                                    </div>

                                    <div class="school">
                                        <img src="img/school_logos/csab.png" alt="CSAB Logo" class="school-logo">
                                        <div class="school-details">
                                            <h3>Colegio de San Agustin Bacolod</h3>
                                        </div>
                                    </div>
                                    <div class="school">
                                        <img src="img/school_logos/unor.png" alt="UNOR Logo" class="school-logo">
                                        <div class="school-details">
                                            <h3>Univeristy of Negros Occidental - Recoletos</h3>
                                        </div>
                                    </div>
                                </div>
                        </div>
                    </article>
                    
<!-- san carlos city schools                 -->
                    <article class="article-schools">
                        <figure class="pic-container">
                            <img class="city-pic" 
                                src="img/city_logos/san carlos.png" 
                                alt="Scholarship Opportunities">
                        </figure>
                        <div class="art-des">
                            <div class="art-title">
                                <h2>San Carlos City</h2>
                            </div>
                            <div class="des-art">
                                <p>Browse available scholarships from local schools inside San Carlos, including eligibility requirements, application steps, and key details to help you start your journey.</p>
                            </div>
                            <div class="schools hidden">
                                <div class="school">
                                    <img src="img/school_logos/csr.jpg" alt="csr Logo" class="school-logo">
                                    <div class="school-details">
                                        <h3> Colegio de Santa Rita de San Carlos </h3>
                                    </div>
                                </div>
                                <div class="school">
                                    <img src="img/school_logos/cstr.png" alt="cstr Logo" class="school-logo">
                                    <div class="school-details">
                                        <h3> Colegio de Sto. Tomas </h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </article>

<!-- kabankalan city schools -->         
                    <article class="article-schools">
                        <figure class="pic-container">
                            <img class="city-pic" 
                                src="img/city_logos/kabankalan.png" 
                                alt="Scholarship Opportunities">
                        </figure>
                        <div class="art-des">
                            <div class="art-title">
                                <h2>Kabankalan City</h2>
                            </div>
                            <div class="des-art">
                                <p>Browse available scholarships from local schools inside Kabankalan, including eligibility requirements, application steps, and key details to help you start your journey.</p>
                            </div>
                             <div class="schools hidden">
                                <div class="school">
                                    <img src="img/school_logos/cpsu.jpg" alt="cpsu Logo" class="school-logo">
                                    <div class="school-details">
                                        <h3> Central Philippine State University - Kabankalan </h3>
                                    </div>
                                </div>
                                <div class="school">
                                    <img src="img/school_logos/fbc.jpg" alt="fbc Logo" class="school-logo">
                                    <div class="school-details">
                                        <h3> Fellowship Baptist College </h3>
                                    </div>
                                </div>
                        </div>
                    </article>

                    
                    <article class="article-schools">
                        <figure class="pic-container">
                            <img class="city-pic" 
                                src="img/city_logos/talisay.png" 
                                alt="Scholarship Opportunities">
                        </figure>
                        <div class="art-des">
                            <div class="art-title">
                                <h2>Talisay City</h2>
                            </div>
                            <div class="des-art">
                                <p>Browse available scholarships from local schools inside Talisay City, including eligibility requirements, application steps, and key details to help you start your journey.</p>
                            </div>
                            <div class="schools hidden">
                                <div class="school">
                                    <img src="img/school_logos/chmsu.png" alt="CHMSU Logo" class="school-logo">
                                    <div class="school-details">
                                        <h3> Carlos Hilado Memorial State University </h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </article>

                    </div>
                </div>
            
                <div class="sub-nav">
                    <div class="sub-container">
                        <div class="subs">
                            <a href="#">School Scholarship</a>
                        </div>
                        <div class="subs">
                            <a href="#">City Scholarship</a>
                        </div>
                        <div class="subs">
                            <a href="#">Private Scholarship</a>
                        </div>
                        <div class="subs">
                            <a href="#">Government Scholarship</a>
                        </div>
                    </div>
                </div>
            </div>
        </main>
        <script src = "assets/scholarship/school.js"></script> 
    </body>
</html>